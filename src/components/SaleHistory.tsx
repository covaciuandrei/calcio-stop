...
const SaleHistory: React.FC<Props> = ({ sales, products, archivedProducts, setSales }) => {
  ...
  const getProductDetails = (productId: string) => {
    ...
    return `${product.name} - ${product.type} - ${product.size} (${product.season}) - ${product.playerName} #${product.equipmentNumber > 0 ? product.equipmentNumber : "-"}`;
  };
  ...
  const handleEditClick = (sale: Sale) => {
    ...
    setEditDate(new Date(sale.date).toISOString().slice(0, 16));
  };
